...
for(let i=0;i<terrGeo.attributes.position.count;i++){
  ...
  // Iniciar la altura h con un ruido base MUY BAJO (para el piso)
  let h = n * 0.1; // Piso con ruido ligero (0 a 0.06)
  // (Si quieres el piso 100% plano, cambia la línea de arriba por: let h = 0;)

  // Sumar las "montañas" (ridge)
  h += ridge;
 ...
 // Aguas abajo (x>0): PISO A NIVEL 0
 // Ya no hundimos el piso. Lo dejamos al nivel base (0 + ruido).
 // const downstreamDrop = 0.05 * smoothstep(0, VALLEY_L*0.50, x); // ganancia y alcance
 // h -= downstreamDrop;  // ¡Ya no restamos!
...
const X_FACE_UP = 2.15;               // podés volver a 0 si querés
const EPS_EDGE  = 0.005;

/* ========================================================================== */
/* EMBALSE (solo aguas arriba)                                               */
/* ========================================================================== */
const RES_W = 20;
const RES_H = 10;
...
/* Opcional: borde/espuma (invisible si edgeH=0) */
const edgeH = 0.0;
...
const markers   = [];  // sólo esferas (pulsan)
const pickables = [];  // todo lo “clickeable” (incluye caseta)
...
/* ---- Piezómetros: línea uniforme a lo largo de la presa (sobre la corona) */
(function addPiezometersOnCrest(){
  const N = 9;                      // cantidad de piezómetros
  const marginZ = 0.35;                   // margen para no pegar al borde
  const zStart = -CFRD.L/2 + marginZ;
  const zEnd   =  CFRD.L/2 - marginZ;
  const x = CFRD.runUp + CFRD.crest*0.5;    // sobre la corona (centro)
  const y = CFRD.H + 0.10;
...
/* ---- Caseta: cubo amarillo aguas abajo, en el piso (lado seco) */
(function addCaseta(){
  const toeX = CFRD.runUp + CFRD.crest + CFRD.runDn; // pie aguas abajo de la presa
  const caseta = new THREE.Mesh(
    new THREE.BoxGeometry(0.45, 0.35, 0.45), // Altura Y = 0.35
    new THREE.MeshStandardMaterial({ color:0xf0da4c, roughness:0.5, metalness:0.1 })
  );
  // La altura del cubo es 0.35, para que su base esté en Y=0, su centro debe estar en Y = 0.35 / 2
  caseta.position.set(toeX, 0.35 / 2, 0.0); // Nivel del suelo (base en Y=0)
...
function animate(){
  ...
  // Piezómetros “respiran” (la caseta NO)
  markers.forEach((m,i)=>{
